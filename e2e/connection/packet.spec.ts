...
import { openConnectionManuallySendText } from '../e2e_helpers';
...
let scene: string = process.env.INWORLD_E2E_CHARACTER_TEXT!;
...
test('[Packet] Packet properties are correct for new connection with text sent', async () => {
  ...
  const connection = await openConnectionManuallySendText(
    key,
    name,
    scene,
    'Hi',
